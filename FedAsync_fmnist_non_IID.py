...
from fedlab.contrib.dataset import PartitionedCIFAR10,PartitionedFMNIST
...
if __name__ == '__main__':
    ...
    csvfile = open('result/FedAsync_fmnist_unbalance_noiid_nowarm_16_338.csv', mode='a', newline='')
    ...
    for epoch in range(global_epoch):
        ...
        # 客户端训练
        train_data = hetero.get_dataloader(random.randint(0,client_num-1), batch_size=batch_size)
        optimizer = optim.SGD(model.parameters(), lr=lr, weight_decay=weight_decay)
        ...
        # 列表最后的模型是最新的模型
        for param, param_server in zip(model.named_parameters(), params_prev_list[-1]):
            name = param[0]
            layer = param[1]
            update_layer = param_server * (1 - alpha_scaled) + layer * alpha_scaled
            model.state_dict()[name].copy_(update_layer.clone())
