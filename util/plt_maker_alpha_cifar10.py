...
# 获取文件某一列数据
with open("../result/FedDgc_cifar10_unbalance_noiid_nowarm_100_0.2.csv") as data:
...
with open("../result/FedDgc_cifar10_unbalance_noiid_nowarm_100_0.4.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedDgc_acc_4 = []
    fedDgc_loss_4 = []
...
with open("../result/FedDgc_cifar10_unbalance_noiid_nowarm_100_0.8.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedDgc_acc_8 = []
    fedDgc_loss_8 = []
...
# plt.savefig('../pic_zh/cifar10_alpha', dpi=300)
plt.show()
