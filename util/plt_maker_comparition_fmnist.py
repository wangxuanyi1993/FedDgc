...
with open("../result/FedAsync_fmnist_unbalance_noiid_nowarm.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedAsync_acc = []
    fedAsync_loss = []
...
with open("../result/FedTed_fmnist_unbalance_noiid_nowarm.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedTed_acc = []
    fedTed_loss = []
...
with open("../result/FedSAFA_fmnist_unbalance_noiid_nowarm_1_norho_4.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedSAFA_acc = []
    fedSAFA_loss = []
...
fedAvg_x = fedAvg_time(mu,sigma,agg,100)
...
# plt.savefig('../pic/fmnist_comparition_addSAFA', dpi=300)
# plt.savefig('../pic_zh/fmnist_comparition', dpi=300)
plt.show()
