...
# 获取文件某一列数据
with open("../result/FedAsync_cifar10_unbalance_noiid_nowarm_16.csv") as data:
...
with open("../result/FedTed_cifar10_unbalance_noiid_nowarm_16.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedTed_acc = []
    fedTed_loss = []
...
with open("../result/FedDgc_cifar10_unbalance_noiid_nowarm_100_16.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedDgc_acc = []
    fedDgc_loss = []
...
with open("../result/FedSAFA_cifar10_unbalance_noiid_nowarm_1_norho_24.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    fedSAFA_acc = []
    fedSAFA_loss = []
...
# plt.savefig('../pic/cifar10_delay_16_addSAFA', dpi=300)
# plt.savefig('../pic_zh/cifar10_delay_16', dpi=300)
plt.show()
