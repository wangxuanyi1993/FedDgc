...
with open("../result/FedAvg_cifar10_unbalance_noiid_ascend_nowarm.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    ascend_acc = []
    ascend_loss = []
...
with open("../result/FedAvg_cifar10_unbalance_noiid_descend_nowarm.csv") as data:
    reader = csv.reader(data)
    header = next(reader)
    descend_acc = []
    descend_loss = []
...
# plt.savefig('../pic_zh/Fedavg_cifar10_ascend_uniform_descend', dpi=300)
plt.show()
